...
public class Day20 extends Solution {
    ...
    public Object getPart1Solution() {
        final List<Tile> tiles = getInput();

        Tile.buildGraph(tiles);
        return tiles.stream().filter(a -> a.isCorner()).map(a -> a.id).reduce((a, b) -> a *= b).get();
    }
    ...
    public Object getPart2Solution() {
        ...
        for (int f = 0; f < 2; f++) {
            for (int r = 0; r < 4; r++) {
                ...
                if (count > 0) {
                    return totalRoughness - count * 15;
                }
